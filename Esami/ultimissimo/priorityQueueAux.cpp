...
const List EMPTYList = NULL; // la size della coda e' zero

// Funzioni ausiliarie che non appartengono al TDD 

// calcola la priorita' maggiore secondo la definizione data
bool hasMaxPriority(const Patient &p1, const Patient &p2) {
  if (p1.triage > p2.triage)
    return true; 
  else if (p1.triage < p2.triage)
         return false;
  ...
  return (p1.registrationTime < p2.registrationTime);
}

// aggiunge il paziente
void addPatient(const Patient &p, Node *curr, Node *prev, PriorityQ& pq) {
    Node *aux = new Node; 
    aux->info = p;
    aux->next = curr;
    if (prev != EMPTYList) 
...
PriorityQ priorityQueue::createEmptyPQ() {
  return EMPTYPriorityQ;
}

bool priorityQueue::isEmpty(const PriorityQ& pq) {
  return ((pq.size == 0) && (pq.list == EMPTYList));
} 
...
void printAidOrder(PriorityQ pq, int startTime) { 
   cout << "\nIl medico inizia a visitare all'istante " << startTime << endl;
   Patient tmp;
   int currentTime = startTime;
   while (!isEmpty(pq)) {
      findMax(pq, tmp);
      prettyPrint(tmp, currentTime, currentTime+tmp.requiredAidTime);
      currentTime = currentTime+tmp.requiredAidTime+1;
      deleteMax(pq);
    }
}
